...
BEGIN=$KEZD
END=$TART
FAJL="inditasi_parameterek3.conf"
KESZLET="adatok3.text"
NAPLO1=0
NAPLO2=0
SZAMOTIS=0
...
then
    ...
    if grep "^SORRENDBEN" $FAJL > /dev/null
    then
        SBAN=`grep "^SORRENDBEN" $FAJL | head -n 1 | cut -d "=" -f2`
...
#
    if grep "^ELSO" $FAJL > /dev/null
    then
        LS=`grep "^ELSO" $FAJL | head -n 1 | cut -d "=" -f2`
        if [ $LS = "MAGYAR" ]
        then
            ELOBB="MAGYAR"
        fi
...
#
    if grep "^HANGOS" $FAJL | head -n 1 | cut -d "=" -f2 > /dev/null
    then
        HG=`grep "^HANGOS" $FAJL | head -n 1 | cut -d "=" -f2`
        if [ $HG = "ANGOL" ]
        then
            AVOICE=1
            MVOICE=0
...
#
    ...
    if grep "^ELSO_IDO" $FAJL | head -n 1 | cut -d "=" -f2 > /dev/null
    then
        EI=`grep "^ELSO_IDO" $FAJL | head -n 1 | cut -d "=" -f2`
        if [ $EI = "ENTER" ]
        then
            ELSOENTER=1
            ELSOSZUNET=1
...
#
    ...
    if grep "^MASODIK_IDO" $FAJL | head -n 1 | cut -d "=" -f2 > /dev/null
    then
        MI=`grep "^MASODIK_IDO" $FAJL | head -n 1 | cut -d "=" -f2`
        if [ $MI = "ENTER" ]
        then
            MASODIKENTER=1
            MASODIKSZUNET=1
...
#
    ...
    if grep "^KESZLET" $FAJL | head -n 1 | cut -d "=" -f2 > /dev/null
    then
        KESZLET=`grep "^KESZLET" $FAJL | head -n 1 | cut -d "=" -f2`
        if [ ! -f $KESZLET ]
        then
...
do
    ...
    then
        ...
        ANGOL_SOR=`grep ^$S $KESZLET | cut -c 7- | cut -d "#" -f1 | sed s/" sg "/" something "/g | sed s/" sy "/" somebody "/g`
        MAGYAR_SOR=`grep ^$S $KESZLET | cut -c 7- | cut -d "#" -f2`
        clear
        if [ $SZAMOTIS -eq 1 ]
        then
            echo $S
